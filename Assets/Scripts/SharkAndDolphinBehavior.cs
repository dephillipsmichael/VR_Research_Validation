...
public class SharkAndDolphinBehavior : Singleton<FishBehavior> {
    ...
    List<Fish> sharkFishObjList = new List<Fish>();
    ...
    void Update() {
        ...
        if (timeOffset > (durationInSeconds + startOffset)) 
        {
        ...
        for (int i = 0; i < sharkFishObjList.Count; i++)
        {
            Fish fish = sharkFishObjList[i];
...
/*
    ...
    void CreateSharkOrDolphin()
    {
        ...
        float time = 3f;
        float fishPerSecond = (fishPerMinute / 60.0f) / 2;
        while (time < durationInSeconds)
        {
            GameObject prefabToUse = sharkPrefab;
            String name = SHARK_NAME;
            if (random.Next(10) <= 4)
            {
                prefabToUse = dolphinPrefab;
                name = DOLPHIN_NAME;
            }
            float sharkTimeRandom = sharkTime + UnityEngine.Random.Range(-5.0f, 5.0f);
            Fish sharkFishObj = new Fish(sharkTimeRandom, randomPathIdx(), false);
            Vector3 startingPos = availablePathList[sharkFishObj.availablePathIdx].path[0];
            sharkFishObj.rotationCorrection = false;

            GameObject shark = Instantiate(prefabToUse, startingPos, new Quaternion(0, 0, 0, 0));

            sharkFishObj.name = name + (int)time;
            shark.name = sharkFishObj.name;
            shark.SetActive(false);

            if (shark.transform.Find("Flake") != null)
            {
                shark.transform.Find("Flake").gameObject.SetActive(false);
            }

            sharkFishObjList.Add(sharkFishObj);
            sharkObjList.Add(shark);
